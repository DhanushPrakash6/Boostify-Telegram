        ...
        <div className="w-full flex flex-col gap-6 pt-5">
          ...
          <div className="bg-gradient-to-r from-green-600 to-emerald-600 rounded-2xl p-6 shadow-lg">
            ...
            <div className="text-center">
              <div className="text-4xl font-bold mb-2">
                {referralInfo ? '$' +referralInfo.referralEarnings.toFixed(2) : (
          ...
          <div className="bg-gradient-to-r from-purple-600 to-blue-600 rounded-2xl p-6 shadow-lg">
              ...
              <div className="flex items-center justify-between">
                <span className="text-3xl font-bold tracking-wider">
                  {referralInfo ? referralInfo.referralCode : (
                    <div className="h-10 bg-white bg-opacity-20 rounded-lg animate-pulse w-48"></div>
                  )}
                </span>
                <button
                  onClick={() => referralInfo && copyToClipboard(referralInfo.referralCode)}
                  disabled={!referralInfo}
                  className="bg-white bg-opacity-20 hover:bg-opacity-30 rounded-lg px-4 py-2 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {copied ? "Copied!" : "Copy"}
                </button>
              </div>
            ...
            <div className="bg-white bg-opacity-10 rounded-xl p-4">
              ...
              <div className="flex items-center justify-between">
                <span className="text-sm text-gray-300 truncate mr-3 flex-1">
                  {referralInfo ? referralInfo.referralLink : (
                    <div className="h-4 bg-white bg-opacity-20 rounded animate-pulse"></div>
                  )}
                </span>
                <button
                  onClick={() => referralInfo && copyToClipboard(referralInfo.referralLink)}
                  disabled={!referralInfo}
                  className="bg-white bg-opacity-20 hover:bg-opacity-30 rounded-lg px-3 py-1 text-sm transition-all duration-200 flex-shrink-0 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  Copy
                </button>
              </div>
          ...
          <div className="bg-gradient-to-r from-indigo-600 to-purple-600 rounded-2xl shadow-lg p-6 mb-[120px]">
            ...
            {referralInfo && referralInfo.referredUsers.length > 0 ? (
              ...
              <div className="text-center">
                {referralInfo ? (
                  <>
                    <div className="text-6xl mt-3 mb-4 flex justify-center items-center">
                  ...
                  <>
                    <p className="mt-3 text-gray-300">
                      <div className="h-4 bg-white bg-opacity-20 rounded animate-pulse w-32 mx-auto"></div>
                    </p>
                    <p className="text-sm text-gray-400 mt-2">
                      <div className="h-3 bg-white bg-opacity-20 rounded animate-pulse w-48 mx-auto"></div>
                    </p>
                  </>
                )}
              </div>
