...
app.post('/api/createCryptoPayment', async (req, res) => {
  const { userId, cryptoType, amount, usdValue, userInfo } = req.body;
  
  try {
    const connection = await clientPromise;
    const db = connection.db("Boostify");
    const usersCollection = db.collection("Users");
    const paymentsCollection = db.collection("CryptoPayments");
    ...
    // In a real implementation, you would integrate with actual crypto payment processors
    // like Coinbase Commerce, BitPay, or similar services
    ...
    setTimeout(async () => {
      try {
        ...
        // Add coins to user's balance (1 USD = 1 coin for simplicity)
        const coinsToAdd = Math.floor(usdValue);
        await usersCollection.updateOne(
          { _id: Number(userId) },
          { $inc: { coins: coinsToAdd } }
        );

        console.log(`Payment completed: ${paymentId} - Added ${coinsToAdd} coins to user ${userId}`);
      } catch (error) {
...
app.get('/api/getPaymentStatus/:paymentId', async (req, res) => {
...
app.get('/api/getUserPayments/:userId', async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ success: false, message: "Error fetching user payments" });
  }
});

// Get real-time crypto prices
app.get('/api/cryptoPrices', async (req, res) => {
  try {
    const response = await axios.get('https://api.coingecko.com/api/v3/simple/price?ids=bitcoin,ethereum,tether,binancecoin,solana&vs_currencies=usd');
    ...
    res.status(500).json({ success: false, message: "Error fetching crypto prices" });
  }
});
